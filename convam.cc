...
// #include "tensorflow/core/util/mirror_pad_mode.h" // Often not needed or
// moved
#include "tensorflow/core/framework/bounds_check.h"
#include "tensorflow/core/platform/macros.h"
#include "tensorflow/core/util/padding.h"
#include "tensorflow/core/util/tensor_format.h"
// #include "tensorflow/core/util/use_cudnn.h" // CUDA specific, likely
// deprecated/internal #include "tensorflow/core/platform/stream_executor.h" //
// Internal, use carefuly
...
// Backward compatibility for Status (if using very new TF that defaults to
// tsl::Status) but usually tensorflow::Status is available.
...
Status ConvBackpropExtractAndVerifyDimensionV2(
  ...
  TF_RETURN_IF_ERROR(GetWindowedOutputSize(dim->input_size, dim->filter_size,
                                           dim->dilation, dim->stride, padding,
                                           &out_size, &pad_size));
...
// this implementation does not support padding and diltation currently, please
// padding before using conv Convolution parameters specified by Op attributes.
...
Status ComputeConv2DDimension(const Conv2DParameters &params,
  ...
  TF_RETURN_IF_ERROR(
      GetWindowedOutputSize(input_rows, filter_rows, dilation_rows, stride_rows,
                            params.padding, &out_rows, &pad_rows));
  TF_RETURN_IF_ERROR(
      GetWindowedOutputSize(input_cols, filter_cols, dilation_cols, stride_cols,
                            params.padding, &out_cols, &pad_cols));
...
public:
  explicit approx_mul_lut(OpKernelConstruction *context)
...
// cpu specilisation
template <typename T> struct ConvamFunctor<CPUDevice, T> {
  void operator()(const CPUDevice &d, const T *input_data, T *output_data,
                  const int batch, const int out_rows, const int out_cols,
                  const int out_depth, const int stride_cols,
                  const int stride_rows, const int filter_left_offset,
                  const int filter_top_offset, const int filter_rows,
                  const int filter_cols, const int in_depth,
        ...
        for (int out_x = 0; out_x < out_cols; ++out_x) {
          for (int out_channel = 0; out_channel < out_depth; ++out_channel) {
              ...
              for (int filter_x = 0; filter_x < filter_cols; ++filter_x) {
                for (int in_channel = 0; in_channel < in_depth; ++in_channel) {
                  ...
                  if ((in_x >= 0) && (in_x < input_cols) && (in_y >= 0) &&
                      (in_y < input_rows)) {
                    input_value = input_data[(batch_ * input_cols * input_rows *
                                              in_depth) +
            ...
            output_data[(batch_ * out_cols * out_rows * out_depth) +
                        (out_y * out_cols * out_depth) + (out_x * out_depth) +
                        out_channel] = total;
          }
        }
      }
...
template <typename Device, typename T> class ConvamOp : public OpKernel {
public:
  explicit ConvamOp(OpKernelConstruction *context)
      : OpKernel(context), mul_lut_(context) {
  ...
  void Compute(OpKernelContext *context) override {
    ...
    Tensor im2col;
    // int64 const oneinputsize = dimensions.input_rows * dimensions.input_cols
    // * dimensions.in_depth; int64 const oneoutputsize = dimensions.out_rows*
    // dimensions.out_cols * dimensions.out_depth;
    if (dimensions.filter_cols == 1 && dimensions.filter_rows == 1 &&
      ...
      if ((size_t)dimensions.batch <= max_batch) {
        long long int size_shape =
            dimensions.batch * dimensions.in_depth * dimensions.filter_rows *
            dimensions.filter_cols * dimensions.out_rows * dimensions.out_cols;
        OP_REQUIRES_OK(context, context->allocate_temp(
                                    DataTypeToEnum<T>::v(),
                                    TensorShape({size_shape}), &im2col));
      } else {
        long long int size_shape =
            max_batch * dimensions.in_depth * dimensions.filter_rows *
            dimensions.filter_cols * dimensions.out_rows * dimensions.out_cols;
        OP_REQUIRES_OK(context, context->allocate_temp(
                                    DataTypeToEnum<T>::v(),
                                    TensorShape({size_shape}), &im2col));
      }
    } else if (dimensions.filter_rows == dimensions.input_rows &&
               dimensions.filter_cols == dimensions.input_cols &&
               params_.padding == 1) {
      long long int size_shape =
          dimensions.batch * dimensions.in_depth * dimensions.filter_rows *
          dimensions.filter_cols * dimensions.out_rows * dimensions.out_cols;
      ...
      if ((size_t)dimensions.batch <= max_batch) {
        long long int size_shape_x =
            dimensions.batch * dimensions.out_cols * dimensions.out_rows;
        long long int size_shape_y = dimensions.in_depth *
                                     dimensions.filter_rows *
                                     dimensions.filter_cols;
        OP_REQUIRES_OK(context,
                       context->allocate_temp(
                           DataTypeToEnum<T>::v(),
                           TensorShape({size_shape_x, size_shape_y}), &im2col));
      } else {
        long long int size_shape =
            max_batch * dimensions.in_depth * dimensions.filter_rows *
            dimensions.filter_cols * dimensions.out_rows * dimensions.out_cols;
    ...
    if (params_.padding == 1) {
      filter_left_offset =
          ((dimensions.out_cols - 1) * dimensions.stride_cols +
           dimensions.filter_cols - dimensions.input_cols + 1) /
          2;
      filter_top_offset = ((dimensions.out_rows - 1) * dimensions.stride_rows +
                           dimensions.filter_rows - dimensions.input_rows + 1) /
                          2;
      filter_left_offset = filter_left_offset > 0 ? filter_left_offset : 0;
      filter_top_offset = filter_top_offset > 0 ? filter_top_offset : 0;
    } else {
      filter_left_offset = ((dimensions.out_cols - 1) * dimensions.stride_cols +
                            dimensions.filter_cols - dimensions.input_cols) /
                           2;
      filter_left_offset = filter_left_offset > 0 ? filter_left_offset : 0;
      filter_top_offset = ((dimensions.out_rows - 1) * dimensions.stride_rows +
                           dimensions.filter_rows - dimensions.input_rows) /
                          2;
      filter_top_offset = filter_top_offset > 0 ? filter_top_offset : 0;
    }
    ConvamFunctor<Device, T>()(
        context->eigen_device<Device>(), input_data, output_data,
        dimensions.batch, dimensions.out_rows, dimensions.out_cols,
        dimensions.out_depth, dimensions.stride_cols, dimensions.stride_rows,
        filter_left_offset, filter_top_offset, dimensions.filter_rows,
        dimensions.filter_cols, dimensions.in_depth, dimensions.input_cols,
        dimensions.input_rows, filter_data, im2col_data, params_.padding,
        mul_lut_);
  }
...
template <typename T> struct ConvamFilterGradFunctor<CPUDevice, T> {
  void operator()(const CPUDevice &d, const T *input, const T *grad, T *im2col,
                  ...
                  const int stride_cols, const int filter_cols,
      ...
      for (int out_x = 0; out_x < filter_cols; ++out_x) {
        for (int in_channel = 0; in_channel < in_depth; ++in_channel) {
          for (int out_channel = 0; out_channel < out_depth; ++out_channel) {
            ...
            T total(0);
            for (int grad_y = 0; grad_y < (out_rows - 1) * stride_rows + 1;
                 ++grad_y) {
              for (int grad_x = 0; grad_x < (out_cols - 1) * stride_cols + 1;
                   ++grad_x) {
                for (int g_batch = 0; g_batch < batch; ++g_batch) {
                  ...
                  if ((in_x >= 0) && (in_x < input_cols) && (in_y >= 0) &&
                      (in_y < input_rows)) {
                    input_value =
                        input[(g_batch * input_rows * input_cols * in_depth) +
                              (in_y * input_cols * in_depth) +
                              (in_x * in_depth) + in_channel];
                  } else {
                    input_value = T(0);
                  ...
                  const T grad_value =
                      (x & y)
                          ? grad[(g_batch * out_cols * out_rows * out_depth) +
            ...
            output[(out_y * filter_cols * in_depth * out_depth) +
                   (out_x * in_depth * out_depth) + (in_channel * out_depth) +
                   out_channel] = total;
          }
        }
      }
...
public:
  explicit ConvamFilterGradOp(OpKernelConstruction *context)
      : OpKernel(context), mul_lut_(context) {
    string data_format;
  ...
  void Compute(OpKernelContext *context) override {
    ...
    TensorShape filter_shape = {filterdimdata[0], filterdimdata[1],
                                filterdimdata[2], filterdimdata[3]};
    ...
    OP_REQUIRES_OK(context, GetWindowedOutputSizeVerbose(
                                ...
                                /*dilation=*/1, dims.spatial_dims[0].stride,
                                padding_, &dims.spatial_dims[0].output_size,
                                &pad_top, &pad_bottom));
    OP_REQUIRES_OK(context, GetWindowedOutputSizeVerbose(
                                dims.spatial_dims[1].input_size,
                                dims.spatial_dims[1].filter_size,
                                /*dilation=*/1, dims.spatial_dims[1].stride,
                                padding_, &dims.spatial_dims[1].output_size,
                                &pad_left, &pad_right));
    ...
    // VALID Padding
    if (padding_ == 1) {
      filter_top_offset = ((dims.output_size(0) - 1) * stride_rows +
                           dims.filter_size(0) - dims.input_size(0) + 1) /
                          2;
      filter_left_offset = ((dims.output_size(1) - 1) * stride_cols +
                            dims.filter_size(1) - dims.input_size(1) + 1) /
                           2;
      filter_left_offset = filter_left_offset > 0 ? filter_left_offset : 0;
      filter_top_offset = filter_top_offset > 0 ? filter_top_offset : 0;
    } else {
    ...
    OP_REQUIRES_OK(context,
                   context->allocate_temp(
                       DataTypeToEnum<T>::v(),
                       TensorShape({input_batch * input_channel *
                                    ((grad_height - 1) * stride_rows + 1) *
                                    ((grad_width - 1) * stride_cols + 1) *
    ...
    ConvamFilterGradFunctor<Device, T>()(
        context->eigen_device<Device>(), in_data, grad, im2col_data,
        input_height, input_width, input_batch, input_channel, grad_width,
        grad_height, grad_channel, filter_left_offset, filter_top_offset,
        stride_rows, stride_cols, filter_width, filter_height, out, mul_lut_);
  }
...
#define REGISTER_GPU_FILTERGRAD(T)                                             \
  ...
  REGISTER_KERNEL_BUILDER(Name("ConvamFilterGrad")                             \
                              .Device(DEVICE_GPU)                              \
                              .TypeConstraint<T>("T")                          \
...
template <typename T> struct ConvamInputGradFunctor<CPUDevice, T> {
  void operator()(const CPUDevice &d, const T *grad, T *im2col,
                  const int hole_grad_width, const int hole_grad_height,
                  const int pad_top, const int pad_left, const T *filter,
                  T *rsfilter, const int filter_rows, const int filter_cols,
                  const int out_depth, const int stride_rows,
                  const int stride_cols, const int batch, const int input_rows,
        ...
        for (int out_x = 0; out_x < input_cols; ++out_x) {
          for (int in_channel = 0; in_channel < in_depth; ++in_channel) {
            ...
            for (int filter_y = 0; filter_y < filter_rows; ++filter_y) {
              for (int filter_x = 0; filter_x < filter_cols; ++filter_x) {
                for (int out_channel = 0; out_channel < out_depth;
                  ...
                  const T filter_v =
                      filter[((filter_rows - 1 - filter_y) * filter_cols *
                              in_depth * out_depth) +
                             ((filter_cols - 1 - filter_x) * in_depth *
                              out_depth) +
                             (in_channel * out_depth) + out_channel];
            ...
            output[(ibatch * input_cols * input_rows * in_depth) +
                   (out_y * input_cols * in_depth) + (out_x * in_depth) +
                   in_channel] = total;
          }
        }
      }
...
public:
  explicit ConvamInputGradOp(OpKernelConstruction *context)
      : OpKernel(context), mul_lut_(context) {
    string data_format;
    OP_REQUIRES_OK(context, context->GetAttr("data_format", &data_format));
    OP_REQUIRES(context, FormatFromString(data_format, &data_format_),
  ...
  void Compute(OpKernelContext *context) override {
    ...
    TensorShape input_shape = {indimdata[0], indimdata[1], indimdata[2],
                               indimdata[3]};
    ...
    OP_REQUIRES_OK(context, GetWindowedOutputSizeVerbose(
                                ...
                                /*dilation=*/1, dims.spatial_dims[0].stride,
                                padding_, &dims.spatial_dims[0].output_size,
                                &forw_pad_top, &forw_pad_bottom));
    OP_REQUIRES_OK(context, GetWindowedOutputSizeVerbose(
                                dims.spatial_dims[1].input_size,
                                dims.spatial_dims[1].filter_size,
                                /*dilation=*/1, dims.spatial_dims[1].stride,
                                padding_, &dims.spatial_dims[1].output_size,
                                &forw_pad_left, &forw_pad_right));
    ...
    OP_REQUIRES_OK(context, context->allocate_temp(
                                DataTypeToEnum<T>::v(),
                                TensorShape({output_channel * filter_width *
                                             filter_height * input_channel}),
                                &rsfilter));
    auto grad = out_backprop.flat<T>().data();
    ...
    size_t const block_size = 16;
    size_t const max_batch =
        (65536 * block_size + 1 - block_size) / (input_height * input_width);
    Tensor im2col;
    if ((size_t)input_batch <= max_batch) {
      long long int size_shape = output_channel * filter_width * filter_height *
                                 input_height * input_width * grad_batch;
      OP_REQUIRES_OK(context, context->allocate_temp(DataTypeToEnum<T>::v(),
                                                     TensorShape({size_shape}),
                                                     &im2col));
    } else {
      long long int size_shape = output_channel * filter_width * filter_height *
                                 input_height * input_width * max_batch;
      OP_REQUIRES_OK(context, context->allocate_temp(DataTypeToEnum<T>::v(),
                                                     TensorShape({size_shape}),
                                                     &im2col));
    }
    ...
    ConvamInputGradFunctor<Device, T>()(
        context->eigen_device<Device>(), grad, im2col_data, hole_grad_width,
        hole_grad_height, back_pad_top, back_pad_left, in_data, rsfilter_data,
        filter_height, filter_width, output_channel, stride_rows, stride_cols,
        input_batch, input_height, input_width, input_channel, out, grad_height,
        grad_width, mul_lut_);
  }
...
#define REGISTER_GPU_INPUTGRAD(T)                                              \
  ...
  REGISTER_KERNEL_BUILDER(Name("ConvamInputGrad")                              \
                              .Device(DEVICE_GPU)                              \
                              .TypeConstraint<T>("T")                          \
