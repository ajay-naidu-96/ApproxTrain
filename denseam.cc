...
// #include "tensorflow/core/util/mirror_pad_mode.h" // Deprecated/Moved
...
template <typename T> struct DenseamFunctor<CPUDevice, T> {
...
template <typename Device, typename T> class DenseamOp : public OpKernel {
public:
  explicit DenseamOp(OpKernelConstruction *context)
  ...
  void Compute(OpKernelContext *context) override {
    ...
    DenseamFunctor<Device, T>()(context->eigen_device<Device>(), input_tensor,
                                weights_tensor, output_tensor, batch, units,
                                input_width, mul_lut_);
  }
...
template <typename T> struct DenseamWeightGradFunctor<CPUDevice, T> {
...
template <typename T> struct DenseamInputGradFunctor<CPUDevice, T> {
...
REGISTER_OP("DenseamGrad")
    ...
    .Attr("T: {float, int32}")
    .Attr("mant_mul_lut: string");
template <typename Device, typename T> class DenseamGradOp : public OpKernel {
public:
  explicit DenseamGradOp(OpKernelConstruction *context)
      : OpKernel(context), mul_lut_(context) {}
  void Compute(OpKernelContext *context) override {
    ...
    OP_REQUIRES_OK(context,
                   context->allocate_output(0, input_shape, &grad_input_t));
    OP_REQUIRES_OK(context,
                   context->allocate_output(1, weights_shape, &grad_weights_t));

    int input_width = input_shape.dim_size(1); // Number of values in each
                                               // sample
    ...
    DenseamWeightGradFunctor<Device, T>()(context->eigen_device<Device>(),
                                          input, grad, grad_weights, batch,
                                          units, input_width, mul_lut_);
    DenseamInputGradFunctor<Device, T>()(context->eigen_device<Device>(),
                                         weights, grad, grad_input, batch,
                                         units, input_width, mul_lut_);
  }
