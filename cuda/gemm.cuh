...
#ifdef AMSIMULATOR
#define MULTIPLY(a, b)                                                         \
  (use_posit_lut ? PositLutMul((a), (b), posit_lut, posit_es)                 \
                 : AMsimulator((a), (b), mant_lut, mant_mask, a_shift,          \
                               b_shift, mant_bitwidth))
...
template <typename T>
__global__ void gemm(size_t m, size_t n, size_t k, const T *a, size_t lda,
                     const T *b, size_t ldb, T *c, size_t ldc,
                     cudaTextureObject_t mant_lut, cudaTextureObject_t posit_lut,
                     uint32_t mant_mask, uint8_t a_shift, uint8_t b_shift,
                     uint8_t mant_bitwidth, bool use_posit_lut, int posit_es) {
  T value(0);
  ...
  for (int i = 0; i < (TILE_DIM + k - 1) / TILE_DIM; ++i) {
    ...
    if (i * TILE_DIM + threadIdx.y < k && Col < n) {
      Bs[threadIdx.y][threadIdx.x] =
          b[(i * TILE_DIM + threadIdx.y) * ldb + Col];
    } else {
