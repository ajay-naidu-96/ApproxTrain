...
// #include "tensorflow/core/kernels/matmul_op.h" // Internal kernel header,
// better avoid if possible or keep if needed
...
template <typename Device, typename T> class MatMulAMOp : public OpKernel {
public:
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    if (a.dims() == 2 && b.dims() == 2) {
      OP_REQUIRES(ctx, a.shape().dim_size(1) == b.shape().dim_size(0),
                  errors::InvalidArgument(
                      "Does not meet cols of mat a is equal rows of mat b"));
      ...
      OP_REQUIRES(ctx,
                  a.shape().dim_size(0) == b.shape().dim_size(0) or
                      a.shape().dim_size(0) == 1 or b.shape().dim_size(0) == 1,
                  errors::InvalidArgument(
                      "Does not meet batch of mat a is equal batch of matb"));
      OP_REQUIRES(ctx, a.shape().dim_size(2) == b.shape().dim_size(1),
                  errors::InvalidArgument(
                      "Does not meet cols of mat a is equal rows of mat b"));
      ...
      OP_REQUIRES(ctx, a.shape().dim_size(1) == b.shape().dim_size(1),
                  errors::InvalidArgument(
                      "Does not meet cols of mat a is equal rows of mat b"));
      batch_b = b.shape().dim_size(0);
      row_a = a.shape().dim_size(0);
      ...
      OP_REQUIRES(
          ctx, false,
          errors::InvalidArgument("Input a and b should be 2 dimensional or 3 "
                                  "dimensional and compitable batch size"));
    }
        ...
        if (batch_a == 1 || batch_b == 1) {
          if (batch_a > batch_b) {
            TensorShape out_shape(
                {a.dim_size(0), a.dim_size(1), b.dim_size(2)});
            OP_REQUIRES_OK(ctx, ctx->allocate_output(0, out_shape, &out));
          } else {
            TensorShape out_shape(
                {b.dim_size(0), a.dim_size(1), b.dim_size(2)});
            OP_REQUIRES_OK(ctx, ctx->allocate_output(0, out_shape, &out));
          }
        } else {
    ...
    LaunchMatMul<Device, T>()(ctx->eigen_device<Device>(), a_data, b_data,
                              batch_a, batch_b, row_a, col_a, row_b, col_b,
                              output, mul_lut_);
  }
...
template <typename T> struct LaunchMatMul<CPUDevice, T> {
  void operator()(const CPUDevice &d, const T &a, const T &b, const int batch_a,
                  const int batch_b, const int row_a, const int col_a,
                  const int row_b, const int col_b, T *out,
